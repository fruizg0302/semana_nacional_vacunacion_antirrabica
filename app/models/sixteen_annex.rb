class SixteenAnnex < ActiveRecord::Base
  def self.state_results(state_id)
    self.find_by_sql("select sa.* from sixteen_annexes as sa inner join jurisdictions as j on j.id = sa.jurisdiction_id inner join states as st on j.state_id = st.id where st.id =#{state_id} ")
  end
